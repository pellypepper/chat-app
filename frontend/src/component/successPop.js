...
const SuccessPopup = ({ message, showContinueButton, handleTimeout, url, tempState }) => {
    ...
    if (!visible)
        return null;
    return (<div id="success-popup" className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-gray-800 text-primary rounded-2xl p-8 max-w-md w-full relative shadow-lg">
        <button onClick={handleClose} className="absolute top-3 right-4 text-2xl text-white hover:text-red-400">
          &times;
        </button>
       ...
       {showContinueButton && (<div className="mt-6 text-center">
    <button className="bg-gradient-to-r from-green-500 to-green-600 px-6 py-3 rounded-lg text-white font-semibold hover:shadow-lg transition" onClick={handleContinue}>
      Continue
    </button>
  </div>)}
